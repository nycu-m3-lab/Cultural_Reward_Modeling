...
# Implicit CoT: Instruction based on CulturalAtlas 5 Domains
CULTURAL_FORENSIC_PROMPT = (
    "Task: Cultural Authenticity Assessment.\nEvaluate how well the image aligns with the explicit text prompt and its expected implicit cultural context. Pay attention to contextually accurate details across relevant socio-cultural dimensions, including: family dynamics, social greetings, dining etiquette, religious practices, and festive settings.\nAssess the presence of authentic local artifacts and norms, penalizing culturally incongruous elements or generic visual stereotypes.\n"
)
...
class GeneralRewardDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_cultural_prompt = use_cultural_prompt 
        if self.cls_based:
            self.path_list = []
            self.label_list = []
    ...
    def __getitem__(self, idx):
        if not self.cls_based:
            ...
            active_system_prompt = (
                CULTURAL_FORENSIC_PROMPT if self.use_cultural_prompt 
                else "You are an AI assistant. Please evaluate the images."
            )
            ...
            if not isinstance(prompt, list):
                ...
                prompt = self.tokenizer.apply_chat_template([system_message, prompt_message], tokenize=False, add_generation_prompt=True)[:-22]
                ...
                prompt_c = self.tokenizer.apply_chat_template([system_message, prompt_message_c], tokenize=False, add_generation_prompt=True)[:-22]
                ...
                prompt_r = self.tokenizer.apply_chat_template([system_message, prompt_message_r], tokenize=False, add_generation_prompt=True)[:-22]
