...
class GeneralPreferenceRewardTrainer(ABC):
    ...
    def fit(self, args):
        ...
        for epoch in range(self.epochs):
            ...
            for inputs_batch_c, inputs_batch_r,c_rate_list,r_rate_list in self.train_dataloader:
                if self.margin_loss:
                    c_rate_tensor = torch.tensor(c_rate_list, dtype=torch.float32)
                    r_rate_tensor = torch.tensor(r_rate_list, dtype=torch.float32)
                    scale_factor = 0.2
                    margin = ((c_rate_tensor - r_rate_tensor) * scale_factor).to(torch.cuda.current_device())
                else:
                    margin = None
                ...
                eval_loss_minimum = self.save_logs_and_checkpoints(args, global_step, step_bar, epoch, logs_dict,reward_config=reward_config, eval_loss_minimum=eval_loss_minimum)
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, epoch, logs_dict={}, model_type='phi', reward_config={}, eval_loss_minimum=None):
        ...
        if args.eval_steps > 0 and global_step % args.eval_steps == 0:
            if self.eval_dataloader is not None:
                current_eval_loss = self.evaluate(self.eval_dataloader, global_step)
                
                self.strategy.print(f"\n[Step {global_step}] Evaluation Finished. Loss: {current_eval_loss:.4f}")

        if global_step % args.save_steps == 0:
            should_save = True
            if args.save_best_model is not None and current_eval_loss is not None:
                if eval_loss_minimum is not None and current_eval_loss > eval_loss_minimum:
                    should_save = False 
            ...
            if should_save:
                ...
                if args.lora_rank > 0:
                    self.strategy.save_model_lora(self.model, self.tokenizer, self.add_cross_attention, save_path, reward_config)
                else:
                    self.strategy.save_model(self.model, self.tokenizer, save_path)
                
                self.strategy.print(f" Model saved at step {global_step} (Eval Loss: {current_eval_loss})")
                

                if current_eval_loss is not None:
                    eval_loss_minimum = current_eval_loss
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            loss_sum = 0
            prob_sum = 0
            for inputs_batch_c, inputs_batch_r, c_rate_list, r_rate_list in eval_dataloader:
                chosen_ids = inputs_batch_c['input_ids'].squeeze(1).to(torch.cuda.current_device())
                c_mask = inputs_batch_c['attention_mask'].squeeze(1).to(torch.cuda.current_device())
                c_pixel_value = inputs_batch_c['pixel_values'].squeeze(1).to(torch.cuda.current_device())
                c_img_size = inputs_batch_c['image_sizes'].squeeze(1).to(torch.cuda.current_device())
