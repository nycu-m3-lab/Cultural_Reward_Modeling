...
from llava_reward.datasets import GeneralRewardDataset,GeneralRewardDataset_qwen,GeneralRewardDataset_llava
from llava_reward.models import get_reward_model
from llava_reward.trainer import GeneralPreferenceRewardTrainer
from llava_reward.utils import blending_datasets, get_strategy, get_tokenizer,get_tokenizer_llava,get_tokenizer_qwen
...
def print_trainable_parameters(model, strategy=None):
    trainable_params = 0
    all_param = 0
    ...
    is_rank_0 = True
    if strategy is not None and hasattr(strategy, "is_rank_0"):
...
def train(args):
    ...
    model = get_reward_model(
        ...
        mean_hidden_state=args.mean_hidden_state,
        ...
        is_pretrained_pm=args.is_pretrained_pm,
        ft_projector=args.ft_projector,
        layer_id=args.layer_id,
    )
    
    print_trainable_parameters(model, strategy)

    # configure tokenizer
    if 'llava' in args.pretrain.lower():
        processor,tokenizer = get_tokenizer_llava(args.pretrain, model, "left", strategy, cache_dir=args.cache_dir, use_fast=not args.disable_fast_tokenizer)
        tokenizer.truncation_side = "right"
    elif 'qwen' in args.pretrain.lower():
        ...
        processor,tokenizer = get_tokenizer(args.pretrain, model, "left", strategy, cache_dir=args.cache_dir, use_fast=not args.disable_fast_tokenizer)
        tokenizer.truncation_side = "right"
    ...
    if args.train_split_ratio <= 1:
        ...
        if 'qwen' in args.pretrain.lower():
            train_dataset = GeneralRewardDataset_qwen(train_data,processor=processor,tokenizer=tokenizer, strategy=strategy, is_custom=args.is_custom_dataset, return_prompt_length=args.return_prompt_length,cls_based=args.cls_based, use_cultural_prompt=not args.disable_cultural_prompt) 
        elif 'llava' in args.pretrain.lower():
            train_dataset = GeneralRewardDataset_llava(train_data,processor=processor,tokenizer=tokenizer, strategy=strategy, is_custom=args.is_custom_dataset, return_prompt_length=args.return_prompt_length,cls_based=args.cls_based, use_cultural_prompt=not args.disable_cultural_prompt) 
        else:
            train_dataset = GeneralRewardDataset(train_data,processor=processor,tokenizer=tokenizer, strategy=strategy, is_custom=args.is_custom_dataset, return_prompt_length=args.return_prompt_length,cls_based=args.cls_based, use_cultural_prompt=not args.disable_cultural_prompt) 
        ...
        if eval_data != None:
            if len(eval_data) > 0:
                if 'qwen' in args.pretrain.lower():
                    eval_dataset = GeneralRewardDataset_qwen(eval_data,processor=processor,tokenizer=tokenizer, strategy=strategy, is_custom=args.is_custom_dataset, return_prompt_length=args.return_prompt_length,cls_based=args.cls_based, use_cultural_prompt=not args.disable_cultural_prompt) 
                elif 'llava' in args.pretrain.lower():
                    eval_dataset = GeneralRewardDataset_llava(eval_data,processor=processor,tokenizer=tokenizer, strategy=strategy, is_custom=args.is_custom_dataset, return_prompt_length=args.return_prompt_length,cls_based=args.cls_based, use_cultural_prompt=not args.disable_cultural_prompt) 
                else:
                    eval_dataset = GeneralRewardDataset(eval_data,processor=processor,tokenizer=tokenizer, strategy=strategy, is_custom=args.is_custom_dataset, return_prompt_length=args.return_prompt_length,cls_based=args.cls_based, use_cultural_prompt=not args.disable_cultural_prompt) 
                eval_dataloader = strategy.setup_dataloader(
                    eval_dataset, args.micro_train_batch_size, True, False, eval_dataset.collate_fn,group_size=args.group_size,
                )
    ...
    trainer = GeneralPreferenceRewardTrainer(
        ...
        is_general_preference=args.is_general_preference,
        add_cross_attention=args.add_cross_attention,
        add_unpaired_head=args.add_unpaired_head,
        cls_based=args.cls_based,
        tau=args.general_preference_tau,
        value_head_dim=args.value_head_dim,
    )
    #save after each epoch
    if args.cls_based:
        trainer.cls_fit(args)
    else:
        trainer.fit(args)
    ...
    if strategy.is_rank_0():
        ...
        print(f">>> Executing final packaging, target directory: {final_dir}")
        os.makedirs(final_dir, exist_ok=True)

        if args.lora_rank > 0:
            lora_path = os.path.join(final_dir, "lora")
            os.makedirs(lora_path, exist_ok=True)
            unwrapped_model = strategy.unwrap_model(model)
            unwrapped_model.save_pretrained(lora_path, safe_serialization=True)
            print(">>> [1/4] LoRA weights saved successfully.")
        ...
        print(">>> [2/4] Vision configuration and Tokenizer saved successfully.")


        custom_files = ["configuration_phi3_v.py", "modeling_phi3_v.py", "processing_phi3_v.py"]
        current_dir = os.getcwd() 
        for f in custom_files:
            src = os.path.join(current_dir, f)
            if os.path.exists(src):
                shutil.copyfile(src, os.path.join(final_dir, f))
                print(f">>> [3/4] Architecture script copied: {f}")

        print(f"\nTask complete! All model components are successfully stored in: {final_dir}")
        print("="*50 + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ft_projector", action="store_true", default=False)
    parser.add_argument("--add_unpaired_head", action="store_true", default=False)
    ...
    parser.add_argument("--is_pretrained_pm", type=str, default=None)
    
    # custom arguments added 
    parser.add_argument("--disable_cultural_prompt", action="store_true", default=False, help="Disable Cultural System Prompt for Ablation Variant A")
    ...
    parser.add_argument("--add_img_eos", action="store_true", default=False, help="add_img_eos")
    parser.add_argument("--cls_based", action="store_true", default=False, help="cls_based")
